...
function bindEvents() {
  ...
  dom.weekly.navBtn.addEventListener('click', () => { switchMode('weekly'); dom.weekly.section.scrollIntoView({ behavior: 'smooth', block: 'start' }); });
  dom.modeTabs.forEach((tab) => tab.addEventListener('click', () => switchMode(tab.dataset.mode)));
  dom.modeJumpButtons.forEach((btn) => btn.addEventListener('click', () => switchMode(btn.dataset.openMode)));
